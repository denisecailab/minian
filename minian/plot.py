...
def plot_overlaps(overlaps, subset, kind='box'):
    ...
    #        'non-shock': 'valence'
    #    }, inplace=True)
    overlap_sub = overlaps.loc[overlaps['mappings'].apply(
        lambda i: set(i) <= subset), :]
    if kind == 'box':
        overlap_sub.boxplot(
            column=['freq'], by=['mappings', 'on', 'group'], rot=50)
        gcount = 1
        for gname, group in overlap_sub.groupby(['mappings', 'on', 'group']):
            ...
            color = group['group'].replace({
                'neutral': 'orange',
                'negative': 'blue'
            }).iloc[0]
            plt.plot(
                x, y, marker='o', linestyle='None', mec='k', ms=9, mfc=color)
            for iname, name in enumerate(n):
                plt.annotate(
                    name,
                    xy=(x[iname], y[iname]),
                    xytext=(x[iname] + 0.1, y[iname]))
            gcount += 1
    elif kind == 'bar':
        ovlp_grouped = overlap_sub.groupby(['mappings', 'on', 'group'])
        overlap_mean = ovlp_grouped.apply(
            np.mean).unstack('group').reset_index()
        overlap_std = ovlp_grouped.aggregate(sem).unstack(
            'group').reset_index()
        overlap_mean.set_index(['mappings', 'on']).plot(
            kind='bar', yerr=overlap_std.set_index(['mappings', 'on']))


def plot_overlaps_temporal(overlaps,
                           subset,
                           suppress_same_session=True,
                           ax=None,
                           kind='box'):
    overlap_sub = overlaps.loc[overlaps['inter'].apply(
        lambda x: set([i[0] for i in x]) <= set(subset)), :]
    if suppress_same_session:
        overlap_sub = overlap_sub.loc[overlap_sub['inter'].apply(
            lambda x: x[0][0] != x[1][0]), :]
    if kind == 'box':
        overlap_sub.boxplot(
            column=['freq'], by=['inter', 'on', 'group'], rot=90)
        gcount = 1
...
def plot_dist_vs_corr(dist_centroids,
                      dist_mask,
                      correlations,
                      savepath=None,
                      suffix=''):
...
def plot_dist_hist(dist_centroids, cut_range=(1, 20, 1), restrict_min=True):
    ...
    for id_cut, cut in enumerate(cuts):
        ...
        if restrict_min:
            dist_min0 = np.tile(
                np.min(dist_centroids, axis=1), (dist_centroids.shape[1],
                                                 1)).transpose()
        ...
        plt1.set_title("matches for first spatial matrix. dist_cutoff: " +
                       str(cut))
        plt2.set_title("matches for second spatial matrix. dist_cutoff: " +
                       str(cut))
