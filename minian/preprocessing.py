...
def corr_coeff_pixelwise(varray):
    ...
    def corr(a, axis):
        return np.apply_along_axis(
            lambda t: np.corrcoef(np.split(t, 2)[0], np.split(t, 2)[1])[0, 1], axis, a
        )
...
def detect_brightspot(varray, thres=None, window=50, step=10):
    ...
    Args:
        varray ([type]): [description]
        thres ([type], optional): [description]. Defaults to None.
        window (int, optional): [description]. Defaults to 50.
    ...
    spots = xr.DataArray(varray.sel(frame=0)).reset_coords(drop=True).astype(int)
                ...
                if (
                    ...
                    print(
                        (
                            ...
                            " using threshold: {:03.2f}"
                        ).format(int(ih), int(iw), float(cur_thres)),
                        end="\r",
                    )
...
def detect_brightspot_perframe(varray, thres=0.95):
    ...
    Args:
        varray ([type]): [description]
        thres (float, optional): [description]. Defaults to 0.95.
    ...
    for fid, fm in varray.rolling(frame=1):
        sp = delayed(lambda f: f > f.quantile(thres, interpolation="lower"))(fm)
        spots.append(sp)
    with ProgressBar():
...
def correct_brightspot(varray, spots, window=2, spot_thres=10, inplace=True):
    ...
    Args:
        varray ([type]): [description]
        spots ([type]): [description]
        window (int, optional): [description]. Defaults to 2.
        spot_thres (int, optional): [description]. Defaults to 10.
    ...
    brt_list = [
        HashableDict(
            (dm, int(spots.coords[dm][brt[idm][ib]].values))
    ...
    for ibrt, brt_cord in enumerate(brt_list):
        ...
        for cord in itt.product(*[cord_rg[1] for cord_rg in cur_sur]):
            cur_sur_list.append(
                HashableDict((cur_sur[i][0], cord[i]) for i in range(len(cord)))
            )
        cur_sur = list(set(cur_sur_list) - set(brt_list))
        sur_list.append(cur_sur)
    for ibrt, cur_brt in enumerate(brt_list):
        print("processing spot {:3d} of {:3d}".format(ibrt, len(brt_list)), end="\r")
        if len(sur_list[ibrt]) > 0:
            cur_sur = xr.DataArray(
                ...
                coords=dict(
                    ...
                    **{r: varr_ds.coords[r] for r in red_dim}
                ),
            )
            ...
            print(
                "unable to correct for point {}, coordinates: {}".format(ibrt, cur_brt)
            )
...
def remove_background_old(varray, window=51):
    ...
    Args:
        varray ([type]): [description]
        window (int, optional): [description]. Defaults to 51.
...
def remove_brightspot(varr, thres=3):
    ...
    Args:
        varr ([type]): [description]
        thres (int, optional): [description]. Defaults to 3.
