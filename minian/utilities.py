...
def handle_crash(varr, vpath, ssname, vlist, varr_list, frame_dict):
    ...
    Args:
        ...
        ssname ([type]): [description]
        vlist ([type]): [description]
        varr_list ([type]): [description]
        frame_dict ([type]): [description]
    ...
    if seg1_list and seg2_list:
        ...
        varr1 = darr.concatenate(list(compress(varr_list, seg1_list)), axis=0)
        varr2 = darr.concatenate(list(compress(varr_list, seg2_list)), axis=0)
...
def video_to_tiffs(ipath, opath, iptn="msCam[0-9]+\.avi$", optn="msCam-%05d.tiff"):
    ...
    cmd = 'ffmpeg -i "concat:{}" -pix_fmt rgba -compression_algo raw {}'.format(
        istr, ostr
    )
    try:
        os.makedirs(opath)
...
def create_fig(varlist, nrows, ncols, **kwargs):
    ...
    Args:
        varlist ([type]): [description]
        nrows ([type]): [description]
        ncols ([type]): [description]
    ...
    aspect = kwargs.pop(
        "aspect", varlist[0].sizes["width"] / varlist[0].sizes["height"]
    )
...
def animate_video(varlist, nrows=None, ncols=None, framerate=30, **kwargs):
    ...
    f_init = fct.partial(multi_im, varlist, subidx={"frame": 0}, ax=ax, **kwargs)
...
def multi_im(
    ...
    **kwargs
    ...
    Args:
        varlist ([type]): [description]
        subidx ([type], optional): [description]. Defaults to None.
        ax ([type], optional): [description]. Defaults to None.
        nrows ([type], optional): [description]. Defaults to None.
        ncols ([type], optional): [description]. Defaults to None.
        animated (bool, optional): [description]. Defaults to False.
        tight (bool, optional): [description]. Defaults to True.
    ...
    for ivar, cur_var in enumerate(varlist):
        if subidx:
            ...
            va = cur_var
        if animated:
            ax[ivar].findobj(matplotlib.collections.QuadMesh)[0].set_array(np.ravel(va))
...
def plot_fluorescence(varlist, ax=None, nrows=None, ncols=None, **kwargs):
    ...
    for ivar, cur_var in enumerate(varlist):
        ...
        ax[ivar].fill_between(cur_mean.indexes["frame"], cur_min, cur_max, alpha=0.2)
...
def save_video(movpath, fname_mov_orig, fname_mov_rig, fname_AC, fname_ACbf, dsratio):
    ...
    Args:
        ...
        fname_mov_orig ([type]): [description]
        fname_mov_rig ([type]): [description]
        fname_AC ([type]): [description]
        fname_ACbf ([type]): [description]
        dsratio ([type]): [description]
    """
...
def save_mp4(filename, dat):
    ...
    vw = sio.FFmpegWriter(
        ...
        outputdict={"-r": "30", "-vcodec": "rawvideo"},
    )
...
def scale_varr(varr, scale=(0, 1), inplace=False, pre_compute=False):
    ...
    Args:
        varr ([type]): [description]
        scale (tuple, optional): [description]. Defaults to (0, 1).
        inplace (bool, optional): [description]. Defaults to False.
        pre_compute (bool, optional): [description]. Defaults to False.
    ...
    if inplace:
        ...
        varr_norm = (
            (varr - varr_min) * (scale[1] - scale[0]) / (varr_max - varr_min)
        ) + scale[0]
    return varr_norm
...
def scale_varr_da(varr, scale=(0, 1)):
    ...
    return (
        ...
        * (scale[1] - scale[0])
        / (darr.nanmax(varr) - darr.nanmin(varr))
    ) + scale[0]
...
def resave_varr(path, pattern="^varr_mc_int.tif$"):
    ...
    for itif, tif_path in enumerate(tiflist):
        ...
        if not cur_var.sizes["height"] == 480 or not cur_var.sizes["width"] == 752:
            print("file {} has modified size: {}".format(tif_path, cur_var.sizes))
...
def save_cnmf(
    ...
    Args:
        ...
        save_pkl (bool, optional): [description]. Defaults to True.
        from_pkl (bool, optional): [description]. Defaults to False.
        unit_mask ([type], optional): [description]. Defaults to None.
        meta_dict ([type], optional): [description]. Defaults to None.
        order (str, optional): [description]. Defaults to 'C'.
    ...
    A = xr.DataArray(
        ...
        coords={"height": h, "width": w, "unit_id": range(cnmf.A.shape[-1])},
        dims=["height", "width", "unit_id"],
        name="A",
    )
    C = xr.DataArray(
        cnmf.C,
        coords={"unit_id": range(cnmf.C.shape[0]), "frame": f},
        dims=["unit_id", "frame"],
        name="C",
    )
    S = xr.DataArray(
        cnmf.S,
        coords={"unit_id": range(cnmf.S.shape[0]), "frame": f},
        dims=["unit_id", "frame"],
        name="S",
    )
    YrA = xr.DataArray(
    ...
    b = xr.DataArray(
        ...
        coords={"height": h, "width": w, "background_id": range(cnmf.b.shape[-1])},
        dims=["height", "width", "background_id"],
        name="b",
    )
    f = xr.DataArray(
        cnmf.f,
        coords={"background_id": range(cnmf.f.shape[0]), "frame": f},
    ...
    if from_pkl:
        ...
        ds = ds.assign_attrs(
            {"unit_mask": unit_mask, "file_path": dpath + os.sep + "cnm.nc"}
        )
...
def save_varr(varr, dpath, name="varr_mc_int", meta_dict=None):
    ...
    Args:
        varr ([type]): [description]
        dpath ([type]): [description]
        name (str, optional): [description]. Defaults to 'varr_mc_int'.
        meta_dict ([type], optional): [description]. Defaults to None.
...
def open_minian(
    ...
    Args:
        ...
        post_process ([type], optional): [description]. Defaults to None.
...
def delete_variable(fpath, varlist, del_org=False):
    ...
    Args:
        fpath ([type]): [description]
        varlist ([type]): [description]
        del_org (bool, optional): [description]. Defaults to False.
...
def save_movies(cnmf, dpath, Y=None, mask=None, Y_only=True, order="C"):
    ...
    Args:
        ...
        Y_only (bool, optional): [description]. Defaults to True.
        order (str, optional): [description]. Defaults to 'C'.
    ...
    if not Y_only:
        ...
        if mask is not None:
            A_dot_C = (
                cnmd["A"].toarray()[:, mask].dot(cnmd["C"][mask, :]).astype(np.float32)
            )
        else:
...
def save_cnmf_from_mat(
    ...
    Args:
        matpath ([type]): [description]
        dpath ([type]): [description]
        vname (str, optional): [description]. Defaults to "ms".
        order (str, optional): [description]. Defaults to 'C'.
    ...
    try:
        ...
        print(
            "No variable with name {} was found in the .mat file: {}".format(
                vname, matpath
            )
        )
        return
    if not dims:
        ...
        dims_coord = (
            np.linspace(0, dims[0] - 1, cnmf.options.d1),
    ...
    C = xr.DataArray(
        cnmf.C,
        coords={"unit_id": range(cnmf.C.shape[0]), "frame": T_coord},
        dims=["unit_id", "frame"],
        name="C",
    )
    S = xr.DataArray(
        cnmf.S,
        coords={"unit_id": range(cnmf.S.shape[0]), "frame": T_coord},
        dims=["unit_id", "frame"],
        name="S",
    )
    if cnmf.b.any():
        ...
        b = xr.DataArray(
            np.zeros(dims + (1,)),
            coords=dict(height=dims_coord[0], width=dims_coord[1], background_id=[0]),
            dims=["height", "width", "background_id"],
            name="b",
        )
    if cnmf.f.any():
        f = xr.DataArray(
            cnmf.f,
            coords={"background_id": range(cnmf.f.shape[0]), "frame": T_coord},
            dims=["background_id", "frame"],
            name="f",
        )
    else:
    ...
    ds = ds.assign_attrs(
        {"unit_mask": unit_mask, "file_path": dpath + os.sep + "cnm_from_mat.nc"}
    )
